...
public class Parser {
	...
	private List<Line> parseLine(String line) {
		...
		case "bnc":
			op = Opcode.BRA;
			BranchType branchType = BranchType.valueOf(opcode.substring(1).toUpperCase());
			return Arrays.asList(new Line(label, Opcode.BRA, branchType, parseValue(params)));
		case "loop":
		case "cmp":
		case "add":
		case "sub":
		...
		case "halt":
			...
			return Arrays.asList(new Line("__halt", Opcode.JUMP,
					OpSource.CONSTANT, null, new Value("__halt")));
		default:
